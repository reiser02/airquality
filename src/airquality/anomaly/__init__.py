...
- ``synthetic``: anomalies (:data:`~airquality.anomaly.benchmark.INJECTION_VARIANT`,
  ...
  synthetic base — and detectors are scored with the supervised metric set
   (VUS-PR et al.); the ensemble uses local rankings on long segments and the
   finite selection mean from the same TSPulse context regime on short segments,
   then averages the station-min-max-normalized top-three detector scores.
