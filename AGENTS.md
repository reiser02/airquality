...
- `airquality.anomaly.benchmark` has two modes (`--mode` / `[anomaly] mode`). **`unlabeled`** (default, matches production): detectors score the real series, scores are binarized with a median + k·MAD threshold, and detectors whose detection rate exceeds `max_detection_rate` (default 7%) are discarded; survivors form the consensus ensemble. **`synthetic`**: anomalies (`combined` shape mix) are injected directly into real segments twice per station (selection + held-out eval seeds). Selection ranks each segment of at least 300 points locally (longest-segment fallback if none qualify), short or locally undefined segments inherit the station-level selection ranking, and held-out evaluation keeps every segment from the configured minimum (default 8). Final metrics pool disjoint finite-score components by station, preserve temporal boundaries, and are multiplied by point coverage; the ensemble backfills its ranked top three point by point before a 2-of-3/2-of-2 vote. The STL synthetic base was removed (see `docs/estudio_inyeccion_stl_2026-07-03.md`). It persists `results.json` and `scores.npz`; plots come from the separate `airquality.anomaly.plot_benchmark_results` script (figure set follows the mode). Label-free rationale: `docs/seleccion_detectores_sin_etiquetas.md`.
...
- Raw block inventory: `uv run python -m airquality.data.block_analysis`. Strategy/imputation support: `uv run python -m airquality.data.block_support_analysis`; plots: `uv run python -m airquality.data.plot_block_support_analysis [run_dir]`.
