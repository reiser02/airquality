...
- `airquality.anomaly.benchmark` has two modes (`--mode` / `[anomaly] mode`). **`unlabeled`** (default, matches production): detectors score the real series, scores are binarized with a median + k·MAD threshold, and detectors whose detection rate exceeds `max_detection_rate` (default 7%) are discarded; survivors form the consensus ensemble. **`synthetic`**: anomalies (`combined` shape mix) are injected directly into real segments twice per station (selection + held-out eval seeds). Selection ranks each segment of at least 300 points locally (longest-segment fallback if none qualify), short segments inherit the station mean ranking, and held-out evaluation keeps every segment from the configured minimum (default 8); each segment's available top-k forms the ensemble. The STL synthetic base was removed (see `docs/estudio_inyeccion_stl_2026-07-03.md`). It persists `results.json` and `scores.npz`; plots come from the separate `airquality.anomaly.plot_benchmark_results` script (figure set follows the mode). Label-free rationale: `docs/seleccion_detectores_sin_etiquetas.md`.
