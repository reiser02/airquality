...
- Training/imputation defaults use processed hourly CSVs under `data/processed/Datos-post-COUTA/*/`; anomaly and forecasting load raw 5-minute stations and run the shared hourly `preprocess()`.
...
- Forecasting benchmark (config `[forecasting]`): `uv run python -m airquality.forecasting.pipeline`; writes `results.csv`, `summary.csv` (deltas vs `raw`), `detection.csv`, and `selection.csv` under `reports/forecasting/<stamp>/`. Detections and backtests are cached content-keyed under `[forecasting] cache_dir` (`use_cache = true`), so interrupted runs resume; figures render separately from the CSVs via `uv run python -m airquality.forecasting.plot_benchmark_results [run_dir]`.
